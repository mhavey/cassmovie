...
public class GoofsFileParser extends MultilineFileParser {
	...
	public GoofsFileParser(String path) {
		super(path, ETLConstants.FIELD_MOVIE_ID, true, SOURCE_NAME,
	...
	protected ParseResult parseOneLine(String line, ImdbRecord currentJSON)
		...
		if (line.startsWith("#")) {
			...
			ImdbRecord newGoof = new ImdbRecord();
			...
			ImdbRecord newJSON = new ImdbRecord();
			newJSON.append(keyFieldName, currentJSON.get(keyFieldName));
			newJSON.append(ETLConstants.FIELD_DOC_TYPE,
					currentJSON.get(ETLConstants.FIELD_DOC_TYPE));
			...
			// it
			if (currentJSON.get(ETLConstants.FIELD_DOC_SUBTYPE) != null) {
			...
			if (currentJSON.get(ETLConstants.FIELD_DOC_SUBTYPE) == null
					|| currentJSON.get(ETLConstants.FIELD_DOC_TEXT) == null) {
				throw new ETLException(ETLConstants.ERR_MALFORMED_LINE,
			...
			String existingDocText = (String) (currentJSON
