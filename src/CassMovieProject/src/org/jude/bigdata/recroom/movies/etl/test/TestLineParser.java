...
public class TestLineParser {
	...
	public static void main(String[] args) throws ETLException {
		...
		String path = "/home/mhavey/imdbetl/imdb";
		...
		// contrib files
		if (1 == 1) {
			runParser(new AkaNamesFileParser(path));
			runParser(new BiographiesFileParser(path));
		}

		// role files
		if (1==1) {
			runParser(new ActorsFileParser(path));
			runParser(new ActressesFileParser(path));
			runParser(new CinematographersFileParser(path));
			runParser(new ComposersFileParser(path));
			runParser(new CostumeDesignersFileParser(path));
			runParser(new DirectorsFileParser(path));
			runParser(new DistributorsFileParser(path));
			runParser(new EditorsFileParser(path));
			runParser(new MiscellaneousCompaniesFileParser(path));
			runParser(new MiscellaneousFileParser(path));
			runParser(new ProducersFileParser(path));
			runParser(new ProductionCompaniesFileParser(path));
			runParser(new ProductionDesignersFileParser(path));
			runParser(new SpecialEffectsCompaniesFileParser(path));
			runParser(new WritersFileParser(path));
		}
