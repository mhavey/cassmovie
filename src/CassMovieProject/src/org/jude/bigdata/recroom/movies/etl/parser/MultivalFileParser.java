...
public abstract class MultivalFileParser extends ImdbLineParser {
	...
	public ImdbRecord parseLine(String line) throws ETLException {
		...
		if (lastMovieID != null) {
			...
			// new movie, so I must return the one I just had
				...
				ImdbRecord myRet = valuesJSON(lastMovieID, lastValues);
				lastMovieID = movieID;
				lastValues = null;
				lastValues = new HashSet<String>();
				lastValues.add(currVal);
				return myRet;
			}
		} else {
		...
		while (true) {
			...
			// found in this line
				...
				ImdbRecord json = valuesJSON(lastMovieID, lastValues);
	...
	/**
	 * Construct json from movie and values. Subclasses can override if they
	 * need control
	 * 
	 * @param movieID
	 * @param vales
	...
	public ImdbRecord valuesJSON(String movieID, Set<String> values) {
		...
		json.append(valuesFieldName, values);
		return json;
	}
