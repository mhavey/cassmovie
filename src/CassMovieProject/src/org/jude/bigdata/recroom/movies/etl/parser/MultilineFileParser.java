...
public abstract class MultilineFileParser extends ImdbLineParser {
	...
	public static class ParseResult {
		ImdbRecord json = null;
		ImdbRecord newCurrentJSON = null;
		boolean returnNow = false;

		public ParseResult(ImdbRecord json, boolean returnNow) {
			super();
			this.json = json;
		 ...
		 * 
		 */
		public ParseResult(ImdbRecord json, ImdbRecord newCurrentJSON) {
			super();
			this.json = json;
		...
		public ImdbRecord getNewCurrentJSON() {
			return this.newCurrentJSON;
		}
	}

	String lastKey = null;
	ImdbRecord lastJSON = null;
	...
	protected abstract ParseResult parseOneLine(String line,
			ImdbRecord currentJSON) throws ETLException;
	...
	public ImdbRecord parseLine(String line) throws ETLException {
		...
		while (true) {
			...
			ImdbRecord nextJSON = result.getJSON();
	 ...
	 */
	@Override
	public ImdbRecord checkMore(String line) throws ETLException {
		if (lastKey == null) {
			return null;
		}
		logger.info(this.getClass().getName()
	 ...
	 * @param currentJSON
	 */
	protected void validateKey(ImdbRecord currentJSON) throws ETLException {
		if (lastKey == null || currentJSON.get(keyFieldName) == null
				|| !lastKey.equals(currentJSON.get(keyFieldName))) {
	 ...
	 * @throws ETLException
	 */
	String keyField(ImdbRecord j) throws ETLException {
		Object k = j.get(keyFieldName);
		if (k == null) {
